...
void main() {
  ...
  group('fromMap', () {
    test('valid map should return RelationshipBarDocument', () {
      ...
      RelationshipBarDocument result =
          RelationshipBarDocument.fromMap(map, userID, firestore);
  ...
  group('firestoreGetLatest', () {
    test('should get barDoc with greatest timestamp', () async {
      ...
      await RelationshipBarDocument.getUserBarsFromID(userID, firestore)
          ...
          .set(expected);
      RelationshipBarDocument? result =
          await RelationshipBarDocument.firestoreGetLatest(userID, firestore);
  ...
  group('firestoreGetAll', () {
    test('should get all barDocs', () async {
      ...
      List<RelationshipBarDocument?> result =
          await RelationshipBarDocument.firestoreGetAll(userID, firestore);
  ...
  group('firestoreSetList', () {
    test('should add each barDoc in list to database', () async {
      ...
      await RelationshipBarDocument.firestoreSetList(
          userID, expected, firestore);
      List<RelationshipBarDocument?> result =
          await RelationshipBarDocument.getUserBarsFromID(userID, firestore)
              .get()
              .then((value) =>
                  value.docs.map((element) => element.data()).toList());
  ...
  group('firestoreUpdateColumns', () {
    test('should update barDoc in database', () async {
      ...
      await RelationshipBarDocument.getUserBarsFromID(userID, firestore)
          .doc(barDocID)
          .set(barDocument);
      await barDocument.firestoreUpdateColumns(
          {RelationshipBarDocument.columnTimestamp: expectedTimestamp});
      RelationshipBarDocument? result =
          await RelationshipBarDocument.getUserBarsFromID(userID, firestore)
              .doc(barDocID)
              .get()
  ...
  group('firestoreAddBarList', () {
    test('adds barDoc with barList to database', () async {
      RelationshipBarDocument? newBarDoc =
          await RelationshipBarDocument.firestoreAddBarList(userID, bars,
              firestore: firestore);
      List<RelationshipBar>? result =
          await RelationshipBarDocument.getUserBarsFromID(userID, firestore)
              .doc(newBarDoc?.id)
              .get()
