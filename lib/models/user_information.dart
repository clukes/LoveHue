...
class UserInformation {
  ...
  static Future<UserInformation?> firestoreGetFromID(String userID, FirebaseFirestore? firestore) async {
    ...
    UserInformation? info = await getUserInDatabaseFromID(userID, firestore).get().then((snapshot) => snapshot.data()).catchError((error) {
  ...
  Future<void> deleteUserData(BuildContext context, FirebaseAuth auth, AuthenticationInfo authenticationInfo) async {
    ...
    if (userID != null && userID == this.userID) {
      try {
        ...
        batchPromises.addAll(await deleteCollection(RelationshipBarDocument.getUserBarsFromID(userID, firestore), firestore: firestore));
  ...
  Future<void> setupUserInDatabase(UserInfoState userInfoState) async {
    ...
    userInfoState.latestRelationshipBarDoc = RelationshipBarDocument.firestoreAddBarListWithBatch(userID, defaultBars, batch, firestore);
